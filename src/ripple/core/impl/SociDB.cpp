           ...
           std::string const& connectionString)
{
    ...
    else if (beName == "mysql")
    {
        ...
        soci::mysql_session_backend *backend = dynamic_cast<soci::mysql_session_backend *> (s.get_backend ());
        if (backend)
        {
            //int timeout = 5;
            my_bool reconnect = 0;
            //mysql_options(backend->conn_, MYSQL_OPT_READ_TIMEOUT, &timeout);
            mysql_options(backend->conn_, MYSQL_OPT_RECONNECT, &reconnect);
        }
    }
#endif
    else if (beName == "empty")
        s.open(soci::empty, connectionString);
    else
...
//    throw std::logic_error ("");
    ...
    auto const& type = s.get_backend_name ();
    if (type == "sqlite3")
    {
        // This function will have to be customized when other backends are added
        if (auto conn = getConnection (s))
    ...
    {
        std::string sql =
        "SELECT Round(Sum(data_length + index_length) / 1024) size FROM information_schema.tables;";
        boost::optional<std::size_t> cur;
        s << sql, soci::into (cur);
        return *cur;
    }
    else
