    ...
    : type_ (type)
{
    ...
    boost::filesystem::path pPath = useTempFiles || type == Type::None
    ? "" : type == Type::MySQL ? strName : (setup.dataDir / strName);
    
    std::string strType = (useTempFiles || type == Type::Sqlite) ? "sqlite" : type == Type::MySQL ? "mysql" : "empty";
    
    open (session_, strType, pPath.string());
