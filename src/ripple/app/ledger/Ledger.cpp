        ...
        getApp().family(), deprecatedLogs().journal("SHAMap")))
{
    ...
    auto const id = *parseBase58<AccountID>("sNwiRpiZGbEnL4mtYq8316tUyHdXGkuENW");
...
bool Ledger::saveValidatedLedger (bool current)
{
    ...
    try {
        if (getApp().getTxnDB ().getType () != DatabaseCon::Type::None) {
            ...
            *db << boost::str (deleteTrans2 % info().seq);
            ...
            for (auto const& vt : aLedger->getMap ())
            {
                ...
                *db <<
                   (STTx::getMetaSQLInsertReplaceHeader (getApp().getTxnDB ().getType ()) +
                    vt.second->getTxn ()->getMetaSQL (
                        info().seq, vt.second->getEscMeta (), info_.closeTime) + ";");
            }
        ...
        // catch mysql error
        WriteLog(lsFATAL, Ledger) << "Mysql insert accountTransaction exception. " << e.what();
        getApp().getLedgerMaster().failedSave(info().seq, info().hash);
        ...
        if (getApp().getTxnDB ().startReconnection ())
        {
            try
            {
                ...
                WriteLog (lsWARNING, Ledger) << "Mysql reconncetion success";
            }
            catch ( std::exception const& e)
            {
                WriteLog(lsFATAL, Ledger) << "Mysql has gone away. And reconnection failed. " << e.what();
                getApp().getTxnDB ().finishReconnection ();
            }
        }
        getApp().pendingSaves().erase(info().seq);
