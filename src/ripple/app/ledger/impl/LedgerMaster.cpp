...
public:
    ...
    */
    ...
    void checkAccept (Ledger::ref ledger)
    {
        ...
        // For Dividend.
        {
            DividendMaster& dm = getApp().getDividendMaster();
            if (getApp().getJobQueue().getJobCount(jtDIVIDEND) > 0)
            {
                m_journal.debug << "Dividend job passed.";
                return;
            }
            getApp().getJobQueue().addJob(jtDIVIDEND,
                                        "DividendMaster::getMissingTxns",
                                        std::bind (
                                                   &DividendMaster::checkAndRunTxns,
                                                   &dm));
        }
    }
