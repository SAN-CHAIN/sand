...
static int runShutdownTests (Config& config)
{
    ...
    for (int i = 0; i < numShutdownIterations; ++i)
    {
        std::cerr << "\n\nStarting server. Iteration: " << i << "\n"
                  << std::endl;
        std::unique_ptr<Application> app (make_Application (config,
                                                            deprecatedLogs()));
...
int run (int argc, char** argv)
{
    ...
    {
        return runShutdownTests (getConfig());
    }
    ...
    if (iResult == 0)
    {
        if (!vm.count ("parameters"))
        {
            ...
            std::unique_ptr <Application> app (make_Application (getConfig(),
                                                                 deprecatedLogs()));
            setupServer ();
            startServer ();
        }
