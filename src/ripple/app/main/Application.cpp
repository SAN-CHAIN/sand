...
public:
    ...
    ApplicationImp (
        ...
        , m_dividendMaster (make_DividendMaster(m_logs.journal("DividendMaster")))
    ...
    bool initSqliteDbs ()
    {
        ...
        DatabaseCon::Setup setup = setup_DatabaseCon (m_config);
        auto const& trasactionDatabse = m_config.section (SECTION_TX_DB);
        ...
        {
            auto const& params = m_config.section (SECTION_TX_DB);
    ...
    void setup ()
    {
        ...
        if (getApp().getTxnDB().getType() == DatabaseCon::Type::Sqlite) {
...
make_Application (
                  ...
                  Logs& logs)
{
    return std::make_unique <ApplicationImp> (config, logs);
}
