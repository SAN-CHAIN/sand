...
const char* TxnDBInit[] =
{
    ...
    "CREATE TABLE IF NOT EXISTS AccountReference (  \
        Account     CHARACTER(64) PRIMARY KEY,                  \
        Referee     CHARACTER(64),                  \
        Contribute  INTEGER DEFAULT 0,              \
        CreateTime  INTEGER DEFAULT 0,              \
        ModifyTime  INTEGER DEFAULT 0               \
    );",
    "CREATE INDEX IF NOT EXISTS AccountReferenceIndex ON \
        AccountReference(Account, Referee);",
    "END TRANSACTION;"
};
...
const char* TxnDBInitMySQL[] =
{
    ...
    "CREATE TABLE IF NOT EXISTS AccountReference (  \
        Account     CHARACTER(64) PRIMARY KEY,      \
        Referee     CHARACTER(64),                  \
        PartsPower  FLOAT DEFAULT 1,                \
        CreateTime  INTEGER DEFAULT 0,              \
        ModifyTime  INTEGER DEFAULT 0               \
        );",
    "CREATE INDEX AccountReferenceIndex ON          \
        AccountReference(Account, Referee);",
    
    "CREATE TABLE IF NOT EXISTS AccountPower(      \
        DumpDate    CHARACTER(12),                 \
        Account     CHARACTER(64),                 \
        LedgerSeq   BIGINT UNSIGNED,               \
        MainPower   BIGINT UNSIGNED,               \
        VicePower   BIGINT UNSIGNED,               \
        SanAge      BIGINT UNSIGNED,               \
        Th          BIGINT UNSIGNED,               \
        TempIndex   BIGINT UNSIGNED,               \
        CreateTime  INTEGER DEFAULT 0,             \
        PRIMARY KEY(DumpDate, Account)             \
    );",
    "CREATE INDEX AccountPowerIndex ON             \
        AccountPower(DumpDate, Account);",
    
    "CREATE TABLE IF NOT EXISTS AccountIncentive(           \
        IncentiveDate       CHARACTER(12),                  \
        Account             CHARACTER(64),                  \
        LedgerSeq           BIGINT UNSIGNED,                \
        ExcitationIndex     BIGINT UNSIGNED DEFAULT 0,      \
        PartsPower          FLOAT DEFAULT 1,                \
        SanAge              BIGINT UNSIGNED DEFAULT 0,      \
        Status              CHARACTER(1) DEFAULT \'W\',     \
        Transaction         LONGBLOB,                       \
        TotalIncentive      BIGINT UNSIGNED DEFAULT 0,      \
        OwnIncentive        BIGINT UNSIGNED DEFAULT 0,      \
        TotalIndex          BIGINT UNSIGNED DEFAULT 0,      \
        CreateTime          INTEGER DEFAULT 0,              \
        ModifyTime          INTEGER DEFAULT 0,              \
        PRIMARY KEY(LedgerSeq, Account)                     \
    );",
    "CREATE INDEX AccountIncentiveIndex ON                  \
        AccountIncentive(LedgerSeq, Account)",
